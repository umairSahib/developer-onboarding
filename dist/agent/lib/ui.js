...
export function box(title, color = chalk.cyan) {
    const line = "─".repeat(60);
    const style = (text) => color(chalk.bold(text));
    log(`\n${style(`┌${line}┐`)}`);
...
export function askConfirmation(question) {
    return new Promise((resolve) => {
        ...
        const ask = () => confirmRl.question(chalk.yellow(`${question} (y/n): `), (answer) => {
...
export function renderHumanResult(toolName, parsed) {
    if (typeof parsed !== "object" || parsed === null)
        return [String(parsed)];
    if (parsed.error)
        return [`❌ ${toolName}: failed - ${parsed.error}`];
    if (parsed.skipped)
        return [`⏭️  ${toolName}: skipped - ${parsed.reason || "already installed"}`];
    if (toolName === "verify_databases" && parsed.verifications) {
        const lines = ["🧪 Database verification results:"];
        const checks = [
    ...
    if (toolName === "check_installed_versions" && Array.isArray(parsed.versions)) {
        const lines = ["Installed versions report:"];
    ...
    const lines = [];
    if (parsed.phase && parsed.step)
        lines.push(`✅ Phase ${parsed.phase}: ${parsed.step} completed`);
    else if (parsed.phase)
        lines.push(`✅ Phase ${parsed.phase} completed`);
    else
    ...
    if (Array.isArray(parsed.results)) {
        for (const r of parsed.results) {
            ...
            if (r.skipped)
                lines.push(`⏭️  ${name}: already installed (skipped)`);
    ...
    if (parsed.manual_step)
        lines.push(`⚠️  Manual step: ${parsed.manual_step}`);
