...
import { askConfirmation, box, log, renderHumanResult, timestamp } from "./ui.js";
import { exportOnboardingSummary } from "./report.js";
const messages = [{ role: "system", content: SYSTEM_PROMPT }];
export async function chat(userMessage) {
    messages.push({ role: "user", content: userMessage });
    const executionReport = [];
    const agentResultLines = [];
    ...
    if (directTool) {
        ...
        if (directTool === "run_all_phases") {
            ...
            for (const phaseTool of RUN_ALL_PHASE_TOOLS) {
                ...
                let parsedPhase;
                try {
                    parsedPhase = JSON.parse(phaseResult);
                }
                catch {
                    parsedPhase = phaseResult;
                }
        ...
        let parsedDirect;
        try {
            parsedDirect = JSON.parse(result);
        }
        catch {
            parsedDirect = result;
        }
...
export async function startCli() {
    box(`🚀 Provisioning Agent  |  Mode: ${DRY_RUN ? "DRY_RUN ✅" : "REAL ⚠️ "}`, DRY_RUN ? chalk.green : chalk.yellow);
