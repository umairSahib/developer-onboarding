...
export function callMCPTool(toolName: string, args: Record<string, unknown>): string {
  ...
  try {
    const result = execSync(
      `echo '${input.replace(/'/g, "\\'")}' | node src/index.js`,
...
export function callMCPToolWithProgress(toolName: string, args: Record<string, unknown>) {
  const spinner = ora({
